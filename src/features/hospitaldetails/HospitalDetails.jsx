...
import { useAddBedsMutation, useGetHospitalDetailsByIdQuery, useLazyGetAllHospitalsQuery, useLazyGetHospitalDetailsByIdQuery } from "../../services/hospitalApi";
...
<div>
        ...
        }
        {
            !isLoading && (
                        ...
                        bedtypes.map((t,i)=>{
                            ...
                            <br />
                            {
                                beds[t].map((bed)=>{
                                    return( 
                                    <div>
                                    {bed.bedStatus==="open" && <i className="bi bi-clipboard h3 m-2" onClick={()=>{occupybed(bed.bedId)}}></i>}
                                    {bed.bedStatus==="occupied" && <i className="bi bi-clipboard-fill h3 m-2" onClick={()=>{occupybed(bed.bedId)}}></i>}
                                    </div>
                                    )
                                })
                            }
                ...
                <button onClick={()=>{updateHospitalBeds()}}>Book Bed</button>
                </div>
                )
        }
